...
export const get_category_list_by_slug = async (slug) => {
  ...
  try {
    ...
    const serviceList = await query(
      `SELECT * FROM services WHERE category = ? AND is_deleted = ? ORDER BY updated_at`,
      [categoryId[0].id, 0]
    );
...
export const store = async (body, files) => {
  ...
  try {
    ...
    if (Array.from(files.keys()).length) {
      ...
      if (og_image && og_alt) {
        // upload files here
        let response = await uploadToServer(og_image, "categories");
      ...
      if (featured_image && alt) {
        // upload files here
        let response = await uploadToServer(featured_image, "categories");
