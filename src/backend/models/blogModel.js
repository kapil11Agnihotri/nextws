...
import { uploadToServer } from "../helpers/uploadToServer";
...
export const store = async (body, files) => {
  ...
  try {
    ...
    let date = moment(new Date(body?.publish_date)).format('DD/MMM/YYYY');
    ...
    if (Array.from(files.keys()).length) {
      ...
      if (og_image && og_alt) {
        // upload files here
        let response = await uploadToServer(og_image, "blogs");

        if (response.code === 201 && response.data != null) {
          og_image = response.data[0];
      ...
      if (featured_image && alt) {
        // upload files here
        let response = await uploadToServer(featured_image, "blogs");

        if (response.code === 201 && response.data != null) {
          featured_image = response.data[0];
        } else return response;
      }
    ...
    const blogs = await query(
      `INSERT INTO blogs (id, slug, title, description, category, short_description, featured_image, alt, blog_tags, meta_title, meta_description, keywords, structured_data, tags, og_title, og_description, og_image, og_alt, publish_date, status, blog_type, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        id,
        body?.slug,
        body?.title,
...
export const update = async (id, body, files) => {
  ...
  try {
    ...
    if (Array.from(files.keys()).length) {
      ...
      if (files?.get("og_image") && body?.og_alt) {
        // upload files here
        let response = await uploadToServer(
          files?.get("og_image"),
          "blogs"
        );
    ...
    const blogs = await query(
      `UPDATE blogs SET slug = ?, title = ?, description = ?, category = ?, short_description = ?, featured_image = ?, alt = ?, blog_tags = ?, meta_title = ?, meta_description = ?, keywords = ?, structured_data = ?, tags = ?, og_title = ?, og_description = ?, og_image = ?, og_alt = ?, publish_date = ?, status = ?, blog_type = ?, updated_at = ? WHERE id = ?`,
      [
        body.slug,
        body.title,
        body.description,
