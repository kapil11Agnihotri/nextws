...
export const store_page = async (body, files, userId) => {
  ...
  try {
    ...
    if (Array.from(files.keys()).length) {
      ...
      if (og_image && og_alt) {
        // upload files here
        let response = await uploadToServer(og_image, "pages");
      ...
      if (featured_image && alt) {
        // upload files here
        let response = await uploadToServer(featured_image, "pages");
...
export const file_upload = async (body, files, baseURL, folder) => {
  ...
  try {
    ...
    if (Array.from(files.keys()).length) {
      // upload files here
      let response = await uploadToServer(files?.get("image"), "pages");
