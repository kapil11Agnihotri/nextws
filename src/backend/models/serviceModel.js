  ...
  try {
    ...
    if (Array.from(files.keys()).length) {
      ...
      if (files?.get("tech_images") && body?.tech_images_alt) {
        ...
        let response = await uploadToServer(tech_images_files, "services");
      ...
      if (og_image && og_alt) {
        // upload files here
        let response = await uploadToServer(og_image, "services");
      ...
      if (featured_image && alt) {
        // upload files here
        let response = await uploadToServer(featured_image, "services");
...
export const file_upload = async (body, files, baseURL, folder) => {
  ...
  try {
    ...
    if (Array.from(files.keys()).length) {
      // upload files here
      let response = await uploadToServer(files?.get("image"), "services");
