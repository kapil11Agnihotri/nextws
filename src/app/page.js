...
export default async function Home({ pageData }) {
  ...
  const homeProductsListData = productsdata[0]?.serviceList;

  const productSectionData = homeProductsListData.filter((item) => {
    return JSON?.parse(item?.featured_for)?.find((value) => {
      return value === "Home Page";
    });
  });

  const result = await get_category_list_by_slug("Services");
  const servicesdata = result?.data;
  const homeServicesListData = servicesdata[0]?.serviceList;
  

  const serviceSectionData = homeServicesListData.filter((item) => {
    return JSON?.parse(item?.featured_for)?.find((value) => {
      return value === "Home Page";
    });
  });
