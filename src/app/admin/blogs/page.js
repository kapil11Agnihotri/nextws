...
const Blogs = () => {
  ...
  const searchItems = ["title", "blog_type_name"];
  ...
  const fetchData = async () => {
    try {
      ...
      const data=res?.data?.map((item)=>{
        item["status"]=item["status"]==2?"Published":"Draft"
        return item
      })
      setBlog(data);
    } catch (error) {
            ...
            >
              ...
              <div className="card" style={{ marginTop: "2%" }}>
                <div className="card-header d-flex justify-content-between">
                  <h6 className="card-title" style={{position:'relative', top:'0.5rem'}}>BLOGS</h6>
                  <div className="d-flex justify-content-end">
                {authPermissions.includes("Blogs-Create") && (
                  <Button text="Add New" link="admin/blogs/add" icon="add" />
                )}
              </div>
                </div>
