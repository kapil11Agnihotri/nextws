...
const AddPage = () => {
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (response.code == 201) {
          ...
          showErrorToast(
            response?.message?.issues[0]?.message ||  response.code + " : Error occurred while creating Page"
          );
        }
      } catch (error) {
  ...
  const handleRowManipulation = (name, index, check) => {
    ...
    if (check) {
      // Add a new row
      rowsData.push({ heading: "", content: "",link: "" });
                      ...
                      {formik.values.servicesSection.card.map((card, index) => (
                        <React.Fragment key={index}>
                          ...
                          <div className='col-md-6 col-sm-12'>
                            <div className=''>
                              <input
                                id={`services_card_link_${index}`}
                                name={`servicesSection.card[${index}].link`}
                                type='text'
                                className='form-control'
                                placeholder={`Card ${index + 1} Link*`}
                                onChange={formik.handleChange}
                                onBlur={formik.handleBlur}
                                value={card.link}
                                style={{ margin: "2%" }}
                      ...
                      {formik.values.productsSection.card.map((card, index) => (
                        <React.Fragment key={index}>
                          ...
                          <div className='col-md-6 col-sm-12'>
                            <div className=''>
                              <input
                                id={`products_card_link_${index}`}
                                name={`productsSection.card[${index}].link`}
                                type='text'
                                className='form-control'
                                placeholder={`Card ${index + 1} Link*`}
                                onChange={formik.handleChange}
